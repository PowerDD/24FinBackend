		...
		shop: 1,
			...
			if (data.success && data.correct) {
				...
				for( i=0; i<data.result.length; i++ ) {
					...
					var html = '<tr><td><i class="fa fa-home pointer i-popover" data-toggle="popover" data-placement="right" data-content="'+result.address+'"></i> ';
					...
					html += result.name;
					html += '<span id="compositebar-'+result.id+'" class="compositebar pull-right">'+result.price3.toFixed(0)+','+result.price2.toFixed(0)+','+result.price1.toFixed(0)+','+result.price0.toFixed(0)+'</span></td>';
					html += '<td class="text-center">'+result.sellPrice+'</td>';
					html += '<td class="text-right">'+((result.price3 == 0) ? '-' : numberWithCommas(result.price3.toFixed(0)))+'</td>';
					html += '<td class="text-right">'+((result.bill3 == 0) ? '-' : numberWithCommas(result.bill3))+'</td>';
					html += '<td class="text-right">'+((result.price2 == 0) ? '-' : numberWithCommas(result.price2.toFixed(0)))+'</td>';
					html += '<td class="text-right">'+((result.bill2 == 0) ? '-' : numberWithCommas(result.bill2))+'</td>';
					html += '<td class="text-right font-bold text-info">'+((result.price1 == 0) ? '-' : numberWithCommas(result.price1.toFixed(0)))+'</td>';
					html += '<td class="text-right">'+((result.bill1 == 0) ? '-' : numberWithCommas(result.bill1))+'</td>';
					html += '<td class="text-right font-bold text-green">'+((result.price0 == 0) ? '-' : numberWithCommas(result.price0.toFixed(0)))+'</td>';
					...
					$('#compositebar-'+result.id).sparkline([result.bill3, result.bill2, result.bill1, result.bill0],
							{composite: true, fillColor: false, lineColor: '#505050'});
				}
...
function loadRegisterInfo(){
	$.post($('#apiUrl').val()+'/member/dealer/register_info', {
		shop: 1,
			...
			if (data.success && data.correct) {
				$('.sp-registerAll').html( numberWithCommas(data.result[0].registerAll.toFixed(0)) );
				$('.sp-registerThisMonth').html( numberWithCommas(data.result[0].registerThisMonth.toFixed(0)) );
				$('.sp-activeAll').html( numberWithCommas(data.result[0].activeAll.toFixed(0)) );
				$('.sp-activeThisMonth').html( numberWithCommas(data.result[0].activeThisMonth.toFixed(0)) );
				var percent = data.result[0].activeThisMonth/data.result[0].activeAll*100;
				$('#chart-register').val( numberWithCommas((percent).toFixed(0)) );
				$('.sp-register_percent').html( numberWithCommas((percent).toFixed(2)) );
				generateChart();
